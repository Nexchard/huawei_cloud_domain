...
class WeworkNotification:
    ...
    def send_domain_notification(self, all_account_domains):
        """发送域名信息通知"""
        if not self.enabled:
            return
        ...
        for bot_name, webhook in self.bots.items():
            if not self.enabled:
                continue
            ...
            for domain in alert_domains:
                ...
                if remaining_days <= 15:
                    ...
                    message += f"- ⚠️ **{domain['domain_name']}**\n"
                else:
                    message += f"- ℹ️ **{domain['domain_name']}**\n"
                
                message += f"  - 到期时间: {domain['expire_date']}\n"
                message += f"  - 剩余天数: {remaining_days}天\n"
            
            self.send_message(message, bot_name) 